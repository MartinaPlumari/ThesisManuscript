...
While it may appear that a processor iterates over every entity simultaneously, the execution is actually performed in batches based on the memory organization of Chunks and Archetypes we previously analyzed. The standard execution flow follows these steps:

\begin{enumerate}
    \item \textbf{Cofiguration:} in the \texttt{ConfigureQueries} function the processor defines its data requirements using one or more query.
...
%ESEMPIO DI CODICE + GUARDA MASS SAMPLE PER REFERENCE E CONFRONTA

\subsubsection{Mass Entity Queries}

The \texttt{FMassEntityQuery} is the mechanism used to filter and fetch data. Queries are highly optimized to perform bitwise comparisons against archetype bitsets, ensuring that the processor only touches relevant memory.

To maximize parallelization and cache efficiency, queries define precisely how they intend to interact with data. These are categorized into Access and Presence requirements.
...
% ESEMPIO CODICE + GUARDA MASS SAMPLE E CONFRONTA

Presence requirements define the filtering rules:

\begin{itemize}
    \item \textbf{All:} it mandates that an entity possess every specified fragments and tags in the query. It acts as a strict logical \texttt{AND} filter.
    \item \textbf{Any:} it mandates that an entity posses at leas one of the specified fragment and tags. It acts as a logical \texttt{OR} filter.
    \item \textbf{None:} it excludes archetypes that contain these components. This is ideal for state changes, for example a Movement processor that needs to ignore entities with a \texttt{Dead} tag.
